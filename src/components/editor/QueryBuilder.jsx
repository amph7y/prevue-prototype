...
import { ArrowUturnLeftIcon, SearchIcon, ClipboardIcon, PlusCircleIcon, MinusCircleIcon } from '../common/Icons.jsx';
import Spinner from '../common/Spinner.jsx';

const QueryBuilder = ({ state, actions }) => {
    const { queries, searchCounts, isSearching, selectedDBs, negativeKeywords, searchFieldOptions } = state;
    const { setStep, handleRunSearch, setNegativeKeywords, setSelectedDBs, setSearchFieldOptions } = actions;
    ...
    return (
        <div className="mt-10">
                    ...
                    <fieldset className="mt-2 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                        {Object.entries(DB_CONFIG).map(([key, { name }]) => (
                            <div key={key} className="relative flex items-start">
                                <div className="flex h-6 items-center"><input id={key} name={key} type="checkbox" checked={selectedDBs[key] || false} onChange={(e) => setSelectedDBs(prev => ({ ...prev, [key]: e.target.checked }))} className="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-600" /></div>
            ...
            <div className="mt-6 space-y-6">
                {Object.entries(queries).map(([dbKey, query]) => {
                        ...
                        <div key={dbKey} className="p-4 border rounded-lg bg-white shadow-sm">
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-center mb-2">
                                ...
                                <div className='flex items-center gap-2'>
                                     ...
                                     <select id={`field-${dbKey}`} value={searchFieldOptions[dbKey]} onChange={(e) => setSearchFieldOptions(prev => ({...prev, [dbKey]: e.target.value}))} className="block w-full rounded-md border-gray-300 py-1.5 pl-3 pr-10 text-gray-900 ring-1 ring-inset ring-gray-300 sm:text-sm">
                                ...
                                <div className='flex items-center justify-end gap-x-2'>
                                     <span className="text-sm font-medium text-gray-700 w-32 text-right">
                                         {searchCounts.loading ? 'Loading...' : (searchCounts[dbKey] !== undefined ? `${Number(searchCounts[dbKey]).toLocaleString()} results` : 'N/A')}
                                     </span>
                                     <button onClick={() => copyToClipboard(query)} className="inline-flex items-center gap-x-1.5 rounded-md bg-white px-2.5 py-1.5 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50">Copy <ClipboardIcon className="h-5 w-5 text-gray-400" /></button>
                                </div>
                            </div>
                            <textarea rows={4} className="block w-full rounded-md border-gray-300 bg-gray-50 font-mono text-sm" value={query || ''} readOnly />
                        </div>
                    )
                })}
            </div>
